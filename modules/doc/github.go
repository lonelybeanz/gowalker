...
var (
	...
	githubRevisionPattern = regexp.MustCompile(`<span class="Button-label color-fg-muted">[a-z0-9A-Z]+</span>`)
...
func getGithubRevision(importPath, tag string) (string, error) {
	...
	i := bytes.Index(data, []byte(`Button-label color-fg-muted`))
	if i == -1 {
	...
	//<span class=\"Button-label color-fg-muted\">a71bb57</span>
	return strings.TrimSuffix(strings.TrimPrefix(string(m[0]), `<span class="Button-label color-fg-muted">`), `</span>`), nil
}
